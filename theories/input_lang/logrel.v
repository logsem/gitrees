...
Lemma logrel_nat_adequacy  Σ `{!invGpreS Σ}`{!statePreG rs Σ} {S} (α : IT (gReifiers_ops rs) natO) (e : expr S) n σ σ' k :
...
Proof.
  ...
  { iIntros (βv). iDestruct 1 as (n'') "[H %]".
    ...
    { change (Nat n') with (IT_of_V (E:=(gReifiers_ops rs)) (RetV n')).
      by iApply f_equivI. }
