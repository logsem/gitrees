...
Proof.
  ...
  - simple refine (λne (us : prodO (prodO unitO stateO) (natO -n> laterO X)),
        let u : prodO natO stateO := update_input (sndO (fstO us)) in
        Some $ ((sndO us) (fstO u), sndO u) : optionO (prodO (laterO X) stateO)).
    intros n [[] s1] [[] s2] [[Hs1 Hs2] Hs]; solve_proper.
  - simple refine (λne (us : prodO (prodO natO stateO) (unitO -n> laterO X)),
        let u : stateO := update_output (fstO (fstO us)) (sndO (fstO us)) in
       Some $ ((sndO us) (), u) : optionO (prodO (laterO X) stateO)).
    intros n [m s1] [m' s2] [[Hs1 Hs2] Hs]; solve_proper.
Defined.
