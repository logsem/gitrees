From gitrees Require Export prelude effects.prng.
Require Import Binding.Resolver Binding.Lib Binding.Set Binding.Auto Binding.Env.
From stdpp Require gmap.

Inductive nat_op := Add | Sub | Mult | Div | Mod | Pow.
...
Fixpoint emap {A B : Set} (f : A [→] B) (e : expr A) : expr B :=
  ...
  | NewPrng => NewPrng
  | DelPrng e => DelPrng (emap f e)
  | Seed e₁ e₂ => Seed (emap f e₁) (emap f e₂)
  | Rand e => Rand (emap f e)
  end
...
Fixpoint ebind {A B : Set} (f : A [⇒] B) (e : expr A) : expr B :=
  ...
  | NewPrng => NewPrng
  | DelPrng e => DelPrng (ebind f e)
  | Seed e₁ e₂ => Seed (ebind f e₁) (ebind f e₂)
  | Rand e => Rand (ebind f e)
  end
...
Definition do_natop (op : nat_op) (x y : nat) : nat :=
  ...
  | Sub => minus x y
  | Mult => mult x y
  | Div => fst $ Nat.divmod x y 0 0
  | Mod => snd $ Nat.divmod x y 0 0
  | Pow => Nat.pow x y
  end.
...
| NewPrngS σ l σ' :
  state_new σ = Some (l,σ') →
  head_step NewPrng σ (Val $ LitPrng l) σ' (1,1)
| DelPrngS σ l σ' :
  state_del σ l = Some ((),σ') →
  head_step (DelPrng (Val $ LitPrng l)) σ (Val $ UnitV) σ' (1,1)
| RandS σ l n σ' :
  state_gen σ l = Some (n, σ') →
...
Proof.
  induction Ki; intros ???; simplify_eq/=; auto with f_equal.
Qed.
...
Fixpoint ectx_compose {S} (K1 K2 : ectx S) : ectx S
  := match K1 with
     | EmptyK => K2
     | DelPrngK K => DelPrngK (ectx_compose K K2)
     | SeedKl K s => SeedKl (ectx_compose K K2) s
     | SeedKs l K => SeedKs l (ectx_compose K K2)
     | RandK K => RandK (ectx_compose K K2)
...
Inductive ty := Tnat | Tunit | Tprng | Tarr : ty → ty → ty.
...
Class DelNotation (A B : Type) := { __del : A -> B }.
Global Instance DelNotationExpr {S : Set} {F : Set -> Type} `{AsSynExpr F}
: DelNotation (F S) (expr S) := { __del e := DelPrng (__asSynExpr e) }.
Global Instance DelNotationK {S : Set}
: DelNotation (ectx S) (ectx S) := { __del K := DelPrngK K }.

Class RandNotation (A B : Type) := { __rand : A -> B }.
Global Instance RandNotationExpr {S : Set} {F : Set -> Type} `{AsSynExpr F}
: RandNotation (F S) (expr S) := { __rand e := Rand (__asSynExpr e) }.
Global Instance RandNotationK {S : Set}
: RandNotation (ectx S) (ectx S) := { __rand K := RandK K }.
...
Notation "'newprng'" := (NewPrng) : syn_scope.
Notation "'delprng' x" := (__del x%syn) (at level 60) : syn_scope.
Notation "'rand' x" := (__rand x%syn) (at level 60) : syn_scope.
Notation "'seed' x y" := (__seed x%syn y%syn) (at level 50) : syn_scope.
...
Notation "'#' '()'" := (UnitV) (at level 60) : syn_scope.
